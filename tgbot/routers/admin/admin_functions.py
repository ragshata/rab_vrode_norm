...
from tgbot.keyboards.reply_main import who_get_mail
...
from tgbot.utils.text_functions import (
    open_profile_admin,
    open_profile_admin_client,
    refill_open_admin,
    purchase_open_admin,
    refill_open_admin_client,
)
...
async def functions_search(message: Message, bot: Bot, state: FSM, arSession: ARS):
    ...
    keyboard.row(ikb("🧑🏻‍💻 Исполнитель", data="find_user")).row(
        ikb("💰 Клиент", data="find_client")
    )
...
# Обработчик для кнопки "Исполнитель"
@router.callback_query(F.data == "find_user")
async def handle_find_user(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    await state.clear()
    await state.set_state("here_search_user")
    await call.message.edit_text(
        "<b>🔍 Отправьте айди/логин пользователя или номер чека</b>"
    )


# Обработчик для кнопки "Клиент"
@router.callback_query(F.data == "find_client")
async def handle_find_client(call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS):
    await state.clear()
    await state.set_state("here_search_client")
    await call.message.edit_text(
        "<b>🔍 Отправьте айди/логин пользователя или номер чека</b>"
    )
...
async def functions_mail(message: Message, bot: Bot, state: FSM, arSession: ARS):
    ...
    await message.answer(
        "<b>❓Кому отправить рассылку?</b>\n", reply_markup=who_get_mail()
    )


@router.message(F.text == "🚨 Всем")
async def functions_mail(message: Message, bot: Bot, state: FSM, arSession: ARS):
    await state.clear()
...
@router.message(F.text == "💰 Заказчикам")
async def functions_mail(message: Message, bot: Bot, state: FSM, arSession: ARS):
    await state.clear()

    await state.set_state("client_here_mail_text")
    await message.answer(
        "<b>📢 Введите текст для рассылки пользователям</b>\n"
        "❕ Вы можете использовать HTML разметку\n",
    )
...
@router.message(F.text == "🧑🏻‍💻 Исполнителям")
async def functions_mail(message: Message, bot: Bot, state: FSM, arSession: ARS):
    await state.clear()

    await state.set_state("user_here_mail_text")
    await message.answer(
        "<b>📢 Введите текст для рассылки пользователям</b>\n"
        "❕ Вы можете использовать HTML разметку\n",
    )
...
@router.message(F.text, StateFilter("here_search_user"))
...
# Принятие айди/логина пользователя или чека для поиска
@router.message(F.text, StateFilter("here_search_client"))
@router.message(F.text.lower().startswith((".find", "find")))
async def functions_search_get(message: Message, bot: Bot, state: FSM, arSession: ARS):
    find_data = message.text.lower()
    ...
    if find_data.isdigit():
        get_client = Clientx.get(client_id=find_data)
        get_refill = Refillx.get(refill_receipt=find_data)
        get_purchase = Purchasesclientx.get(purchase_receipt=find_data)
    else:
        get_client = Clientx.get(client_login=find_data.lower())

    if get_client is None and get_refill is None and get_purchase is None:
        return await message.answer(
            "<b>❌ Данные не были найдены</b>\n"
            "🔍 Отправьте айди/логин пользователя или номер чека",
        )
    ...
    if get_client is not None:
        return await open_profile_admin_client(bot, message.from_user.id, get_client)

    if get_client is not None:
        return await open_profile_admin_client(bot, message.from_user.id, get_client)

    if get_refill is not None:
        return await refill_open_admin_client(bot, message.from_user.id, get_refill)
...
@router.callback_query(
    F.data.startswith("confirm_mail:"), StateFilter("here_mail_confirm")
)
async def functions_mail_confirm(
    ...
    if get_action == "yes":
        ...
        asyncio.create_task(functions_mail_make(bot, send_message, call))
    else:
...
# Принятие текста для рассылки
@router.message(F.text, StateFilter("client_here_mail_text"))
async def functions_mail_get(message: Message, bot: Bot, state: FSM, arSession: ARS):
    await state.update_data(here_mail_text="📢 Рассылка.\n" + str(message.text))

    get_clients = Clientx.get_all()
    ...
    await state.set_state("client_here_mail_confirm")

    await message.answer(
        f"<b>📢 Отправить <code>{len(get_clients)}</code> юзерам сообщение?</b>\n"
        f"{message.text}",
        reply_markup=mail_confirm_finl(),
        disable_web_page_preview=True,
    )
...
# Подтверждение отправки рассылки
@router.callback_query(
    F.data.startswith("confirm_mail:"), StateFilter("client_here_mail_confirm")
)
async def functions_mail_confirm(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    ...
    if get_action == "yes":
        await call.message.edit_text(
            f"<b>📢 Рассылка началась... (0/{len(get_clients)})</b>"
        )

        asyncio.create_task(client_functions_mail_make(bot, send_message, call))
    else:
        await call.message.edit_text("<b>📢 Вы отменили отправку рассылки ✅</b>")


# Сама отправка рассылки
async def client_functions_mail_make(bot: Bot, text: str, call: CallbackQuery):
    users_receive, users_block, users_count = 0, 0, 0
    ...
    for client in get_clients:
        try:
            await bot.send_message(client.client_id, text)
            users_receive += 1
        except:
            users_block += 1
        ...
        if users_count % 10 == 0:
            await call.message.edit_text(
                f"<b>📢 Рассылка началась... ({users_count}/{len(get_clients)})</b>"
            )
    ...
    await call.message.edit_text(
        ded(
            ...
            <b>📢 Рассылка была завершена за <code>{get_unix() - get_time}сек</code></b>
            ➖➖➖➖➖➖➖➖➖➖
            👤 Всего пользователей: <code>{len(get_clients)}</code>
            ✅ Пользователей получило сообщение: <code>{users_receive}</code>
            ❌ Пользователей не получило сообщение: <code>{users_block}</code>
        """
        )
...
# Принятие текста для рассылки
@router.message(F.text, StateFilter("user_here_mail_text"))
async def functions_mail_get(message: Message, bot: Bot, state: FSM, arSession: ARS):
    await state.update_data(here_mail_text="📢 Рассылка.\n" + str(message.text))
    ...
    await state.set_state("user_here_mail_confirm")
...
# Подтверждение отправки рассылки
@router.callback_query(
    F.data.startswith("confirm_mail:"), StateFilter("user_here_mail_confirm")
)
async def functions_mail_confirm(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    ...
    if get_action == "yes":
        ...
        asyncio.create_task(user_functions_mail_make(bot, send_message, call))
    else:
        await call.message.edit_text("<b>📢 Вы отменили отправку рассылки ✅</b>")


# Сама отправка рассылки
async def user_functions_mail_make(bot: Bot, text: str, call: CallbackQuery):
    users_receive, users_block, users_count = 0, 0, 0
...
async def functions_profile_refresh(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
...
async def functions_profile_purchases(
    ...
    if len(get_purchases) < 1:
        return await call.answer(
            "❗ У пользователя отсутствуют выполненные заказы", True
        )

    await call.answer("✅ Последние 10 заказов")
    await del_message(call.message)
    ...
    for purchase in get_purchases:
        ...
        await call.message.answer(
            ded(
                ...
                ✅ Выполненный заказ: <code>{purchase.purchase_position_name} | {purchase.purchase_price}₽</code>
                🕰 Дата выполнения: <code>{convert_date(purchase.purchase_unix)}</code>
                🔗 Заказ: <a href='{link_items}'>кликабельно</a>
            """
            )
        )
...
async def functions_profile_balance_add(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    user_id = call.data.split(":")[1]
...
async def functions_profile_balance_add_get(
    ...
    try:
        await bot.send_message(
            user_id,
            f"<b>💰 На ваш счёт в сервисе зачислено <code>{message.text}₽</code></b>",
        )
    except:
        ...
...
async def functions_profile_balance_set(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    user_id = call.data.split(":")[1]
...
async def functions_profile_balance_set_get(
    message: Message, bot: Bot, state: FSM, arSession: ARS
):
    user_id = (await state.get_data())["here_profile"]
    ...
    if (
        to_number(message.text) < -1_000_000_000
        or to_number(message.text) > 1_000_000_000
    ):
        return await message.answer(
...
# Выдача баланса пользователю
@router.callback_query(F.data.startswith("admin_client_balance_add:"))
async def functions_profile_balance_add(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    user_id = call.data.split(":")[1]
    ...
    await state.update_data(here_profile=user_id)
    await state.set_state("here_profile_add_client")
...
# Принятие суммы для выдачи баланса пользователю
@router.message(F.text, StateFilter("here_profile_add_client"))
async def functions_profile_balance_add_get(
    message: Message, bot: Bot, state: FSM, arSession: ARS
):
    user_id = (await state.get_data())["here_profile"]
    ...
    Clientx.update(
        user_id,
        client_balance=round(get_user.client_balance + to_number(message.text), 2),
        client_give=round(get_user.client_give + to_number(message.text), 2),
    )

    try:
        await bot.send_message(
            user_id,
            f"<b>💰 На ваш счет в сервисе было зачислено <code>{message.text}₽</code></b>",
        )
    except:
        ...

    await message.answer(
        f"👤 Пользователь: <a href='tg://user?id={get_user.client_id}'>{get_user.client_name}</a>\n"
        f"💰 Баланс выдан на <code>{message.text}₽</code>"
    )

    await open_profile_admin_client(bot, message.from_user.id, get_user)


# Изменение баланса пользователю
@router.callback_query(F.data.startswith("admin_client_balance_set:"))
async def functions_profile_balance_set(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    user_id = call.data.split(":")[1]
    ...
    await state.update_data(here_profile=user_id)
    await state.set_state("here_profile_set_client")
...
# Принятие суммы для изменения баланса пользователя
@router.message(F.text, StateFilter("here_profile_set_client"))
async def functions_profile_balance_set_get(
    message: Message, bot: Bot, state: FSM, arSession: ARS
):
    user_id = (await state.get_data())["here_profile"]
    ...
    if to_number(message.text) > get_user.client_balance:
        user_give = get_user.client_give + to_number(message.text)
    else:
        user_give = get_user.client_give

    Clientx.update(
        user_id,
        client_balance=to_number(message.text),
        client_give=user_give,
    )
...
async def functions_profile_user_message(
    call: CallbackQuery, bot: Bot, state: FSM, arSession: ARS
):
    user_id = call.data.split(":")[1]
...
async def functions_profile_user_message_get(
    message: Message, bot: Bot, state: FSM, arSession: ARS
):
    user_id = (await state.get_data())["here_profile"]
    await state.clear()
