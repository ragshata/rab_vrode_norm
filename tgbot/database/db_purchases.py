...
class Purchasesclietnx:
    ...
    def add(
        ...
        with sqlite3.connect(PATH_DATABASE) as con:
            ...
            con.execute(
                ded(f"""
                    INSERT INTO {Purchasesclietnx.storage_name} (
                        client_id,
    ...
    def get(**kwargs) -> PurchasesModel:
        with sqlite3.connect(PATH_DATABASE) as con:
            con.row_factory = dict_factory
            sql = f"SELECT * FROM {Purchasesclietnx.storage_name}"
            sql, parameters = update_format_where(sql, kwargs)
    ...
    def update(purchase_receipt, **kwargs):
        with sqlite3.connect(PATH_DATABASE) as con:
            con.row_factory = dict_factory
            sql = f"UPDATE {Purchasesclietnx.storage_name} SET"
            sql, parameters = update_format(sql, kwargs)
            parameters.append(purchase_receipt)
    ...
    def delete(**kwargs):
        with sqlite3.connect(PATH_DATABASE) as con:
            con.row_factory = dict_factory
            sql = f"DELETE FROM {Purchasesclietnx.storage_name}"
            sql, parameters = update_format_where(sql, kwargs)
