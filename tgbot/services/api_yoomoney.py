...
class YoomoneyAPI:
    ...
    async def error_wallet_admin(self, error_code: str = "Unknown"):
        if not self.skipping_error:
            await send_admins(
                self.bot,
                f"<b>🔮 Юkassa недоступен. Как можно быстрее его замените</b>\n"
    ...
    async def check(self) -> str:
        ...
        if status:
            if len(response) >= 1:
                ...
                return ded(
                    f"""
                    <b>🔮 Юkassa кошелёк полностью функционирует ✅</b>
                    ➖➖➖➖➖➖➖➖➖➖
                ...
                return "<b>🔮 Не удалось проверить Юkassa кошелёк ❌</b>"
        else:
            return "<b>🔮 Не удалось проверить Юkassa кошелёк ❌</b>"
    ...
    async def balance(self) -> str:
        ...
        if status:
            ...
            return "<b>🔮 Не удалось получить баланс Юkassa кошелька ❌</b>"
    ...
    async def authorization_enter(self, get_code: str) -> tuple[bool, str, str]:
        ...
        if "error" in response_data:
            ...
            if error == "invalid_request":
                ...
                return (
                    ...
                    не имеет права запрашивать авторизацию (например, Юkassa заблокировал его 'client_id')</b>
                """
                    ),
                )
                ...
                return (
                    ...
                    ded(
                        f"""
                    <b>❌ В выпуске 'access_token' отказано. Юkassa не выпускал временный токен,
        ...
        if response_data["access_token"] == "":
            return (
                False,
                "",
                "<b>❌ Не удалось получить токен. Попробуйте всё снова.</b>",
            )

        return (
            True,
            response_data["access_token"],
            "<b>🔮 Юkassa кошелёк был успешно изменён ✅</b>",
        )
    ...
    async def bill_check(
        self, receipt: Union[str, int] = None, records: int = 1
    ) -> tuple[int, float]:
        data = {
            "type": "deposition",
            "details": "true",
